...
export const PasskeyModal = () => {
  ...
  }, [encryptedKey]); // Add passkey to the dependency array to detect passkey changes
  ...
  const validatePasskey = async (e: React.MouseEvent<HTMLButtonElement, MouseEvent>) => {
    ...
    if (passkey === process.env.NEXT_PUBLIC_ADMIN_PASSKEY) {
      ...
      setOpen(false);  // Close the modal
      router.push("/admin");  // Navigate to the admin page after modal closes
    } else {
  ...
  const closeModal = () => {
    setOpen(false);
    router.push("/"); // Redirect to home if modal is closed
  };
